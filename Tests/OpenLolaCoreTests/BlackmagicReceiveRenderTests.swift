// Verifies that video reassembler keeps streams independent and counts incomplete fragments.
import Foundation
import Testing
...
func videoReassemblerKeepsStreamsIndependentAndCountsIncompleteFragments() throws {
  let (fragmentsA, fragmentsB) = try independentStreamFragments()
  var reassembler = VideoFrameReassembler(maxActiveFrames: 2)

  let completed = try reassembleInterleavedStreams(
    fragmentsA: fragmentsA,
    fragmentsB: fragmentsB,
    using: &reassembler
  )

  expectIndependentStreams(completed, metrics: reassembler.metrics)
  try expectDuplicateFragmentDrop()
  try expectFlushCountsIncompleteFragments()
}
...
func videoOutputRendererHandlesDeadlineBackpressureContinuityAndRecentLatency() throws {
  ...
  expectDeadlineRendererRejectsLateFrame(packet)
  expectLatestOnlyBackpressure()
  expectContinuityAcceptsSequenceWrap()
  expectRecentLatencyMetrics()
}
...
func receiveRenderSyntheticSmokeAndBlackmagicBoundaryRequirePhysicalEvidence() throws {
  ...
  #expect(renderOutput.framesOutput == 0)
  ...
  #expect(
    report.audioImpact.baselinePlayoutTargetFrames == report.audioImpact.videoPlayoutTargetFrames)
  #expect(report.verdict == .partial)
}

@Test
func blackmagicBoundaryReportsRequirePhysicalEvidenceForPass() throws {
  ...
  var passCandidate = try loadJSONFixture(
  ...
  passCandidate.fragmentation = VideoFragmentationMetrics(
    framesFragmented: 3,
    fragmentsSent: 3,
    maxFragmentsPerFrame: 1,
    maxPayloadBytesPerFragment: 9_000
  )
  passCandidate.reassembly = VideoReassemblyMetrics(
    framesReassembled: 3,
    framesDroppedIncomplete: 0,
    missingFragments: 0,
    lateFragments: 0
  )
  passCandidate.renderOutput = passCandidateRenderOutput()
  passCandidate.receiver.displayedFrames = max(
    0,
    passCandidate.receiver.receivedFrames - passCandidate.receiver.droppedFrames
  )
  passCandidate.blackmagicOutput = fallbackReport
...
private func independentStreamFragments() throws -> (
  [VideoTransportFragment], [VideoTransportFragment]
) {
  ...
  return (fragmentsA, fragmentsB)
}

private func reassembleInterleavedStreams(
  fragmentsA: [VideoTransportFragment],
  fragmentsB: [VideoTransportFragment],
  using reassembler: inout VideoFrameReassembler
  ...
  let completedA = try fragmentsA.dropFirst().compactMap { try reassembler.receive($0) }
  let completedB = try fragmentsB.dropFirst().compactMap { try reassembler.receive($0) }
  return completedA + completedB
}

private func expectIndependentStreams(
  _ completed: [VideoTransportPacket],
  metrics: VideoReassemblyMetrics
) {
  #expect(Set(completed.map(\.streamID)) == [101, 202])
  #expect(completed.allSatisfy { $0.sequenceNumber == 0 })
  #expect(metrics.framesReassembled == 2)
  #expect(metrics.framesDroppedIncomplete == 0)
  #expect(metrics.activeFramesPeak == 2)
}

private func expectDuplicateFragmentDrop() throws {
...
private func expectFlushCountsIncompleteFragments() throws {
...
private func expectDeadlineRendererRejectsLateFrame(_ packet: VideoTransportPacket) {
...
private func expectLatestOnlyBackpressure() {
...
private func expectContinuityAcceptsSequenceWrap() {
  ...
  let renderedMax = continuityRenderer.renderNext(
    renderAtNanoseconds: 400, outputAtNanoseconds: 500)
...
private func expectRecentLatencyMetrics() {
  ...
  for sequenceNumber in UInt64(0)..<10_005 {
    ...
    allRenderedSequencesMatched =
      allRenderedSequencesMatched
      && metricsRenderer.renderNext(
        renderAtNanoseconds: renderAtNanoseconds,
  ...
  #expect(metricsRenderer.metrics.framesOutput == 0)
  ...
  #expect(metricsRenderer.metrics.renderToOutput.maxMicroseconds == 0)
}
