...
product_build_bin_path() {
  local product_name="$1"
  swift build --product "$product_name" --show-bin-path || {
...
let matches = windows.filter { window in
    ...
    return ownerPID == pid || ownerName == displayName
}
...
for window in matches {
    ...
    print("pid=\(ownerPID) owner=\(ownerName) name=\(windowName) bounds=\(bounds)")
}
...
verify_launched_app_surface() {
  ...
  capture_app_screenshot "$evidence_dir" || {
    echo "screenshot capture failed or produced an empty artifact" >&2
    return 1
  }
