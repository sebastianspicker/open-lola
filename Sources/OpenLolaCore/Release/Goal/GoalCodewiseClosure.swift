...
public struct GoalCodewiseDocumentationArea: Codable, Equatable, Sendable {
    ...
    public static let required: [GoalCodewiseDocumentationArea] = [
        .init(path: "docs/architecture", purpose: "sanitized architecture and latency design"),
        .init(path: "docs/roadmap", purpose: "condensed milestone release posture and active blockers"),
        .init(path: "docs/benchmarks", purpose: "benchmark and measurement methodology"),
        .init(path: "docs/background", purpose: "publication-safe background synthesis"),
        .init(path: "docs/compliance", purpose: "clean-room, release, and notice gates"),
        .init(path: "docs/testing", purpose: "verification and surface-probe handoff"),
        .init(path: "docs/diagrams", purpose: "diagram index and public-safe visual map"),
    ]
}
...
private func goalCodewiseRequirements() -> [GoalCodewiseRequirement] {
    let architecture = ["docs/architecture/latency-first-architecture.md", "Sources/OpenLolaCore"]
    ...
    let docs = ["docs/README.md", "docs/current-state.md", "docs/mac-port/README.md"]
    let compliance = ["docs/compliance/README.md", "docs/compliance/release-manifest.md"]
    let validation = ["Tests/OpenLolaCoreTests", "Sources/OpenLolaCore/Evidence/ReportSchemaInventory.swift"]
    let performance = ["Sources/OpenLolaCore/Benchmarks/Performance/PerformanceAuditReport.swift", "docs/architecture/latency-budget.md"]
    return [
        ...
        req(.principleFastestProfileSimpleDirect, "Fastest profile remains simple and direct", .principle, .codeImplemented, ["Sources/OpenLolaCore/Timing/LatencyProfileContracts.swift", "docs/architecture/latency-profiles.md"], "Fastest profiles reject hidden buffering."),
        ...
        req(.principleBenchmarkOrUnvalidated, "Major choices are benchmarked or marked unvalidated", .principle, .codeImplemented, ["Sources/OpenLolaCore/Benchmarks/E2E/E2EBenchmarkReport.swift", "docs/benchmarks/README.md"], "Benchmark reports keep partial verdicts visible."),
        req(.principleSmallMilestones, "Small testable milestones", .principle, .codeImplemented, ["docs/mac-port/README.md"], "The consolidated Mac-port handoff records current status without a duplicate companion fan-out."),
        ...
        req(.complianceResearchToIndependentRequirement, "Research converts to independent requirements", .compliance, .codeImplemented, ["docs/background/validation-and-test-methodology.md", "docs/compliance/README.md"], "Research-to-requirements process is documented in the condensed active docs."),
        ...
        req(.complianceSeparateReverseEngineering, "Keep internal reverse-engineering separate", .compliance, .codeImplemented, ["docs/reverse-engineering/README.md", "private/reverse-engineering/REVERSE_ENGINEERING_EVIDENCE_MATRIX_2026.md"], "Public boundary and internal corpus are separated."),
        ...
        req(.architectureProfessionalAudioRmeMadi, "Professional audio via RME MADI", .architecture, .codeImplemented, audio + ["docs/architecture/audio-rme-madi.md"], "RME/MADI source contracts exist."),
        req(.architectureProfessionalVideoBlackmagicAtem, "Professional video via Blackmagic / ATEM", .architecture, .codeImplemented, video + ["docs/architecture/video-blackmagic-atem.md"], "Blackmagic/ATEM capture and control gates exist."),
        ...
        req(.architectureUDPFirstTransport, "UDP-first realtime media transport", .architecture, .codeImplemented, ["Sources/OpenLolaCore/Network/UDP/UdpPcmPacket.swift", "docs/architecture/p2p-networking.md"], "UDP PCM contracts are primary."),
        ...
        req(.architectureReceiverSideRoutingMixing, "Receiver-side routing and mixing", .architecture, .codeImplemented, ["Sources/OpenLolaCore/Audio/Routing/ReceiverMixSnapshot.swift", "docs/architecture/audio-routing.md"], "Receiver-local mix snapshots exist."),
        req(.architectureRXBufferProfiles, "Optional RX buffer profiles", .architecture, .codeImplemented, ["Sources/OpenLolaCore/Timing/RxBuffering.swift", "docs/architecture/rx-buffering.md"], "RX policies are explicit and benchmarkable."),
        req(.architectureDirectLowLatencyMeasurable, "Direct ultra-low-latency mode remains measurable", .architecture, .codeImplemented, ["Sources/OpenLolaCore/Benchmarks/Latency/LatencyBenchmarkReport.swift", "docs/architecture/latency-profiles.md"], "Latency-profile reports preserve direct-mode measurement."),
        ...
        req(.dodAVTimingDocumented, "AV timing behavior documented", .definitionOfDone, .codeImplemented, ["Sources/OpenLolaCore/Timing/MediaClock.swift", "docs/architecture/av-sync-and-timing.md"], "Audio-master timing policy is documented."),
        req(.dodPerformanceProfilesDocumented, "Performance profiles documented", .definitionOfDone, .codeImplemented, ["docs/architecture/latency-profiles.md", "docs/architecture/benchmark-methodology.md"], "Profiles and benchmarking rules are documented."),
        ...
        req(.artifactArchitectureDocs, "docs/architecture current", .artifact, .codeImplemented, ["docs/architecture"], "Architecture area exists."),
        req(.artifactMilestoneDocs, "docs/roadmap current", .artifact, .codeImplemented, ["docs/roadmap"], "Condensed roadmap area exists."),
        req(.artifactBenchmarkDocs, "docs/benchmarks current", .artifact, .codeImplemented, ["docs/benchmarks"], "Benchmark area exists."),
        req(.artifactResearchDocs, "docs/background current", .artifact, .codeImplemented, ["docs/background"], "Background area exists."),
        req(.artifactReverseEngineeringDocs, "reverse-engineering boundary current", .artifact, .codeImplemented, ["docs/reverse-engineering/README.md"], "Public-safe boundary file exists."),
        req(.artifactComplianceDocs, "docs/compliance current", .artifact, .codeImplemented, ["docs/compliance"], "Compliance area exists."),
        req(.artifactTestingDocs, "docs/testing current", .artifact, .codeImplemented, ["docs/testing"], "Testing area exists."),
        req(.artifactDiagramDocs, "docs/diagrams current", .artifact, .codeImplemented, ["docs/diagrams"], "Diagram area exists."),
        ...
        req(.validationTestsBenchmarkMethod, "Tests or benchmark method", .validation, .codeImplemented, validation + ["docs/benchmarks"], "Tests and benchmark docs exist."),
        req(.validationLatencyImpact, "Latency impact documented", .validation, .codeImplemented, ["docs/architecture/latency-budget.md", "Sources/OpenLolaCore/Benchmarks/Performance/PerformanceAuditReport.swift"], "Latency impact is represented in budget/audit surfaces."),
        req(.validationFailureModes, "Failure modes documented", .validation, .codeImplemented, ["docs/mac-port/risk-register.md", "Sources/OpenLolaCore/Evidence/ReportSchemaInventory.swift"], "Risks and report validators capture failures."),
        req(.validationMilestoneProgress, "Milestone progress updated", .validation, .codeImplemented, ["docs/mac-port/README.md"], "Canonical handoff records source and evidence gates."),
        req(.validationArchitectureDocs, "Architecture docs updated when relevant", .validation, .codeImplemented, ["docs/architecture"], "Architecture index is current."),
        ...
        req(.performanceBenchmarkSensitiveChanges, "Benchmark before/after sensitive changes", .performance, .codeImplemented, ["Sources/OpenLolaCore/Benchmarks/E2E/E2EBenchmarkReport.swift", "docs/benchmarks/e2e-av-benchmark-methodology.md"], "Benchmark contracts exist for sensitive changes."),
        req(.decisionRulePriorityOrder, "Conflict decisions follow GOAL.md priority order", .decisionRule, .codeImplemented, ["GOAL.md", "docs/architecture/latency-first-architecture.md"], "Clean-room correctness and audio latency stay ahead of convenience."),
    ]
}
