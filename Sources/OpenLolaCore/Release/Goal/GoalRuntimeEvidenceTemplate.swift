...
private func audioEvidenceDeliverables() -> [GoalRuntimeEvidenceDeliverable] {
    [
...
private func twoMacRmeMadiBidirectionalEvidence() -> GoalRuntimeEvidenceDeliverable {
    evidence(GoalRuntimeEvidenceDeliverableSpec(
        ...
        reportPaths: ["<run-dir>/m03-rme-loopback-48k-32f.json", "<run-dir>/m05-madi-full-duplex-mac-a.json", "<run-dir>/m05-madi-full-duplex-mac-b.json"],
        validators: ["validate-loopback-report", "validate-madi-full-duplex-report"],
        passCriteria: "Both Macs must validate bidirectional 64-channel RME TX/RX with real Core Audio UIDs and no synthetic device labels."
    ))
}

private func receiverSideRoutingMixingEvidence() -> GoalRuntimeEvidenceDeliverable {
    evidence(GoalRuntimeEvidenceDeliverableSpec(
...
private func directP2PSessionUdpMediaEvidence() -> GoalRuntimeEvidenceDeliverable {
    evidence(GoalRuntimeEvidenceDeliverableSpec(
        id: .directP2PSessionUdpMedia,
        title: "Direct P2P session setup and UDP media path",
        localRunnableSurfaces: ["direct-p2p-two-peer-plan-run", "direct-p2p-session-run", "udp-pcm-route-run", "network-diagnostics-run"],
        requiredPhysicalInputs: ["two routable Mac IPs", "packet capture point", "DSCP policy", "explicit audio/video device IDs"],
        commandTemplates: [
        ...
        reportPaths: ["<run-dir>/m06-direct-p2p-av-plan.json", "<run-dir>/m06-direct-p2p-av-mac-a.json", "<run-dir>/m06-direct-p2p-av-mac-b.json", "<run-dir>/m05-route-receiver.json", "<run-dir>/m05-direct-p2p-network-diagnostics.json"],
        validators: [
        ...
        passCriteria: "Control JSON, session agreement, UDP media, route capture, DSCP evidence, nonzero AV counters, and raw video receive evidence must all come from the same physical route."
    ))
}

private func audioLatencyEvidence() -> GoalRuntimeEvidenceDeliverable {
    evidence(GoalRuntimeEvidenceDeliverableSpec(
        ...
        reportPaths: ["<run-dir>/m03-audio-latency-loopback.json", "<run-dir>/m05-audio-latency-route.json", "<run-dir>/m05-network-diagnostics.json", "<run-dir>/m06-drift-plc-60min.json"],
        validators: ["validate-loopback-report", "validate-route-report", "validate-network-diagnostics-report", "validate-drift-plc-report"],
        passCriteria: "Latency, packet age, jitter, loss, underrun, overrun, and PLC counters must be measured on the accepted physical route."
    ))
}

private func rxBufferBenchmarksEvidence() -> GoalRuntimeEvidenceDeliverable {
    evidence(GoalRuntimeEvidenceDeliverableSpec(
        ...
        passCriteria: "Direct, Small, Adaptive, and Stable/WAN rows must show explicit latency cost on the same measured route."
    ))
}

private func networkAndVideoEvidenceDeliverables() -> [GoalRuntimeEvidenceDeliverable] {
    [
        evidence(GoalRuntimeEvidenceDeliverableSpec(
            id: .blackmagicAtemVideoTxRx,
            title: "Blackmagic/ATEM/DeckLink/UltraStudio video TX/RX",
            localRunnableSurfaces: ["video-capture-inventory", "video-capture-run", "video-transport-run", "atem-readonly-probe"],
            ...
            reportPaths: ["<run-dir>/m08-video-inventory.json", "<run-dir>/m08-video-capture.json", "<run-dir>/m09-video-transport.json", "<run-dir>/m11-atem-readonly.json"],
            validators: ["validate-video-capture-inventory", "validate-video-capture-report", "validate-video-transport-report", "validate-atem-control-report"],
            passCriteria: "Capture and transport must identify real Blackmagic/ATEM hardware and show video does not alter audio timing."
        )),
        evidence(GoalRuntimeEvidenceDeliverableSpec(
            ...
            passCriteria: "The report must prove bounded multi-stream scheduling or explicitly stage the remaining physical stream count."
        )),
    ]
}

private func integrationAndFieldEvidenceDeliverables() -> [GoalRuntimeEvidenceDeliverable] {
    [
        evidence(GoalRuntimeEvidenceDeliverableSpec(
            id: .avTimingRealRuns,
            ...
            reportPaths: ["<run-dir>/m10-integrated-av.json", "<run-dir>/m12-integrated-profile.json", "<run-dir>/m13-e2e-benchmark.json"],
            validators: ["validate-integrated-av-report", "validate-integrated-profile-report", "validate-e2e-benchmark-report"],
            passCriteria: "Real runs must document audio-master timing, AV offset/jitter, video frame age, packet age, and degradation before audio latency changes."
        )),
        evidence(GoalRuntimeEvidenceDeliverableSpec(
            ...
            passCriteria: "OSC and lighting reports must include audio-active comparison and prove control work never runs on audio-critical threads."
        )),
        evidence(GoalRuntimeEvidenceDeliverableSpec(
            ...
            reportPaths: ["<run-dir>/m15-packaging-field.json", "<run-dir>/p05-field-runtime-proof.json", "<run-dir>/field-readiness"],
            validators: ["validate-packaging-field-report", "validate-field-runtime-proof"],
            passCriteria: "PASS requires Developer ID signing, hardened runtime, notarization, stapled ticket, Gatekeeper acceptance, and clean-Mac install/launch evidence."
        )),
    ]
}

private struct GoalRuntimeEvidenceDeliverableSpec {
...
private func evidence(_ spec: GoalRuntimeEvidenceDeliverableSpec) -> GoalRuntimeEvidenceDeliverable {
