...
public struct MadiFullDuplexMetrics: Codable, Equatable, Sendable {
    ...
    public func validate() throws {
        try requireM05NonNegative(transmittedBlocks, "metrics.transmittedBlocks")
        try requireM05NonNegative(transmittedFragments, "metrics.transmittedFragments")
        try requireM05NonNegative(receivedFragments, "metrics.receivedFragments")
        try requireM05NonNegative(completedReceiveBlocks, "metrics.completedReceiveBlocks")
        try requireM05NonNegative(renderedReceiveBlocks, "metrics.renderedReceiveBlocks")
        try requireM05NonNegative(underruns, "metrics.underruns")
        try requireM05NonNegative(overruns, "metrics.overruns")
        try requireM05NonNegative(lateDrops, "metrics.lateDrops")
        try requireM05NonNegative(recoveryEvents, "metrics.recoveryEvents")
        try requireM05NonNegative(Double(txSenderFrameEnd), "metrics.txSenderFrameEnd")
        try requireM05NonNegative(Double(rxPlayoutFrameEnd), "metrics.rxPlayoutFrameEnd")
        try requireM05NonNegative(videoStreamsEnabled, "metrics.videoStreamsEnabled")
        try drift.validate()
...
public struct MadiFullDuplexSessionConfiguration: Codable, Equatable, Sendable {
    ...
    public func validate() throws {
        try requireM05NonEmpty(sessionID, "sessionID")
        try requireM05NonEmpty(localPeerID, "localPeerID")
        try requireM05NonEmpty(remotePeerID, "remotePeerID")
        try requireM05NonEmpty(inputDeviceUID, "inputDeviceUID")
        try requireM05NonEmpty(outputDeviceUID, "outputDeviceUID")
        ...
        try requireM05Positive(packetCount, "packetCount")
        try requireM05Positive(maxTransmissionUnitBytes, "maxTransmissionUnitBytes")
        try requireM05Positive(maxFragmentsPerDeadline, "maxFragmentsPerDeadline")
        try requireM05Positive(metadataRevision, "metadataRevision")
        try requireM05Positive(preallocatedBlockCount, "preallocatedBlockCount")
        try requireM05NonNegative(peerBindTimeoutSeconds, "peerBindTimeoutSeconds")
        try requireM05NonNegative(videoStreamsEnabled, "videoStreamsEnabled")
        try requireM05NonEmpty(receiverMixPolicy, "receiverMixPolicy")
