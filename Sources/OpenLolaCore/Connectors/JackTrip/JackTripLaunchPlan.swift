// Builds validated JackTrip executable arguments for topology, transport, backend, and audio settings.
import Foundation
...
func buildJackTripPlan(
    ...
    let details = try jackTripLaunchPlanDetails(
...
private func jackTripLaunchPlanDetails(
    configuration: ExternalConnectorSessionConfiguration,
    mediaProfile: ExternalConnectorMediaProfile,
    auxiliaryProcesses: [ExternalConnectorAuxiliaryProcessPlan],
    peer: String,
    peerAudioPort: UInt16?
) throws -> ExternalConnectorLaunchPlanDetails {
    ExternalConnectorLaunchPlanDetails(
        execution: .init(
            launchKind: .internalJackTripAudio,
            arguments: try jackTripArguments(
                configuration,
                peer: peer,
                peerAudioPort: peerAudioPort
            ),
        ...
        mediaProfile: mediaProfile,
        protocolFacts: jackTripProtocolFacts(),
        sourceReferences: jackTripSourceReferences(),
        evidenceBoundary: jackTripEvidenceBoundary()
    )
}

private func jackTripSourceReferences() -> [String] {
    [
...
private func jackTripEvidenceBoundary() -> String {
    "Swift-native JackTrip owns the audio packetization path for DEFAULT, JAMLINK, EMPTY, " +
        "WebRTC data-channel, WebTransport datagram, and Opus-extension payloads, and models the " +
        "unauthenticated hub TCP port exchange. Audio-video mode uses UltraGrid as a separate video " +
        "carrier because JackTrip itself is audio-only. Hub TCP, plugin, WebRTC, WebTransport, and " +
        "Opus-extension readiness is not managed Virtual Studio cloud or reference-peer evidence. " +
        "Real JackTrip peer interoperability remains PARTIAL until measured peer capture evidence exists."
}

private func jackTripArguments(
    _ configuration: ExternalConnectorSessionConfiguration,
    peer: String,
    peerAudioPort: UInt16?
) throws -> [String] {
    ...
    try appendJackTripConditionalArguments(
        &arguments,
        configuration: configuration,
        peerAudioPort: peerAudioPort
    )
...
private func appendJackTripConditionalArguments(
...
private func jackTripProtocolFacts() -> [String] {
    [
        "JackTrip UDP audio datagrams carry a DEFAULT header, compact JAMLINK header, " +
            "or the public EMPTY header mode with raw planar PCM only",
        "JackTrip DEFAULT header is the public DefaultHeaderStruct layout copied as " +
            "host-order bytes by the reference implementation",
        "JackTrip JAMLINK header support is bounded to representable 16-bit mono/stereo PCM " +
            "with encoded sample-rate, sequence, timestamp, and samples-per-packet fields",
        "Open LoLa supports little-endian desktop DEFAULT packets with 8/16/24/32-bit PCM, " +
            "explicit channels, sample-rate enum, sequence number, timestamp, and buffer size",
        "Open LoLa supports JackTrip EMPTY header mode only with explicit local sample-rate, " +
            "bit-depth, channel, and frame-size configuration because the wire payload carries no header metadata",
        "JackTrip redundancy concatenates full packets in one datagram; this native slice models " +
            "redundancy as bounded packet copies across UDP-equivalent transports",
        "JackTrip default peer UDP port is 4464; open-lola makes that port explicit in the launch plan",
        "JackTrip WebRTC data-channel mode carries the same packet bytes as the UDP stream " +
            "over an unordered unreliable data channel after WSS signaling",
        "JackTrip WebTransport mode carries the same packet bytes as UDP after a QUIC varint " +
            "quarter-stream prefix in each HTTP/3 datagram",
        "tx-rx mode uses the peer-known direct UDP endpoint because JackTrip P2P audio is " +
            "bidirectional after the first datagram establishes the peer endpoint",
        "JackTrip hub virtual-studio topology is modeled explicitly: hub servers listen for clients, " +
            "hub clients require a configured hub peer, and patch mode is recorded without claiming " +
            "managed Virtual Studio cloud evidence",
        "JackTrip unauthenticated hub TCP handshake exchanges a little-endian client UDP port, " +
            "optional fixed-width client name, and little-endian server UDP port before UDP audio flows",
        "JackTrip audio backend selection is explicit: coreaudio uses the native provider path, " +
            "while jack-graph requires measured JACK graph capture evidence for non-dry runs",
        "JackTrip plugin bridge mode is modeled as an audio-source boundary; plugin-host loading " +
            "remains external to this packet runtime",
        "JackTrip non-PCM audio is modeled as an Open LoLa Opus CELT low-delay extension envelope " +
            "and is not promoted to reference JackTrip interoperability without measured peer evidence",
        "JackTrip is an audio protocol; audio-video mode pairs native JackTrip audio with an auxiliary " +
            "UltraGrid-compatible video process"
    ]
}
...
private func validateJackTripTopology(_ configuration: ExternalConnectorSessionConfiguration) throws {
    ...
    if JackTripCompatibilityRunner.jackTripPeerRequired(configuration), configuration.peer.isEmpty {
