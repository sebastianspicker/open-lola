# SOTA 2026 Open Question Matrix

Date: 2026-05-11
Status: active detailed SOTA routing matrix after docs/mac-port consolidation
Verdict: PARTIAL

This matrix closes the planning ambiguity around the 2026 SOTA research set by
routing every open question to a milestone-owned decision, measurement gate, or
explicit human handoff. It does not claim runtime facts that have not been
measured.

The source-refresh rows below intentionally retain their original checked dates.
Recheck the relevant primary source before implementing a row or promoting a
runtime gate to PASS.
...
## Top-Level Question Disposition

| ID | Disposition | Owning milestone | Required evidence | Blocking scope |
|---|---|---|---|---|
| Q001 | User input required | M01 | Exact reference Macs, OS versions/builds, RME MADI model, driver/firmware, TotalMix state, Core Audio UID values, clock source, channel labels, sample rates, route labels, packet-capture points, DSCP policy, and PASS thresholds recorded from real hardware/route evidence. | Blocks M01 closure, not M00 scaffold. |
| Q002 | Measurement gate | M02-M03 | M02 Core Audio inventory exists and M03 report validation exists; physical analog loopback must still prove accepted sample rates without hidden conversion. | Blocks fastest-mode selection. |
| Q003 | Measurement gate | M03 | M03 report validation now requires 48/96/192 kHz disposition; real loopback matrix must still prove whether higher rates improve corrected one-way latency. | Blocks high-rate default. |
| Q004 | Measurement gate | M05 | M05 route-report source validation, localhost smoke, and PASS-capable physical evidence fields exist; packet captures must still classify each physical route as DSCP honored, rewritten, ignored, harmful, or not tested with reason. | Blocks campus fastest-mode claim. |
...
Use this file before changing a milestone implementation. If a new SOTA source
or standard changes a decision, update the row here first, then update
[README.md](README.md), [open-questions.md](open-questions.md), or
[risk-register.md](risk-register.md) as applicable, then rerun
