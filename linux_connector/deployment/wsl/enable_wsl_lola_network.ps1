# Configure the scoped WSL and Windows firewall state required for connector probes.
...
# Merge only required WSL NAT settings so existing user configuration remains intact.
function Merge-WslConfigText {
    ...
    # Add absent settings without replacing unrelated keys in the [wsl2] section.
...
# Preserve the current WSL configuration before this script changes network settings.
...
# Apply the merged WSL configuration only after ShouldProcess approval.
...
# Allow the selected LoLa UDP ports only on the configured WSL virtual interface.
...
# Add the equivalent Hyper-V policy when this Windows build exposes that API.
...
# Restart WSL when allowed so the updated networking configuration takes effect.
