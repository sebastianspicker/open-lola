...
The canonical same-machine WSL lab guide is
[../../docs/wsl-lab-setup.md](../../docs/wsl-lab-setup.md). This file documents
the deployment helpers in this directory.
...
```bash
python linux_connector/tools/lola_packet_decoder.py lola_probe.pcap
```
...
A dated same-machine Windows/WSL run used `frame_id = sequence + 1`,
`--audio-interval-scale 0.92`, and the optional Npcap UDP relay for packets that
Hyper-V did not deliver into WSL. Re-run the acceptance gates before treating
those settings as evidence for the current revision.
