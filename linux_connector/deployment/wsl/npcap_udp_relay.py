"""Relay Windows/Npcap-captured LoLa UDP payloads into WSL through normal UDP.
...
def send_payload_nonblocking(
    sock: socket.socket | DatagramSender,
    payload: bytes,
    address: tuple[str, int],
) -> bool:
    """Write a UDP payload without blocking the relay event loop."""
    try:
...
def parse_args() -> argparse.Namespace:
    """Parse tshark selection, capture endpoints, relay ports, and stats cadence."""
    ...
    parser.add_argument("--dst-ip", default="192.0.2.30")
...
def validate_relay_args(args: argparse.Namespace) -> None:
    """Validate capture executable, interface, IP addresses, ports, and interval."""
...
def build_tshark_command(args: argparse.Namespace) -> RelayProcessCommand:
    """Build a field-only tshark capture restricted to original LoLa media packets."""
...
def validate_process_argument(value: str, name: str) -> None:
    """Reject empty or control-character-bearing tshark arguments."""
    if not value:
...
def tshark_executable_name(executable: str) -> str:
    """Return the executable basename used by the tshark allowlist."""
...
def validate_udp_port(value: int, name: str) -> None:
    """Reject relay ports outside the valid UDP range."""
...
def resolve_tshark_executable(command: RelayProcessCommand) -> str:
    """Locate tshark executable using the approved local executable lookup."""
...
async def start_tshark_capture(command: RelayProcessCommand) -> Process:
    """Start validated line-buffered tshark output without invoking a shell."""
...
def open_relay_sockets() -> RelaySockets:
    """Create the UDP sockets used by the relay worker."""
...
def require_process_stdout(proc: Process) -> asyncio.StreamReader:
    """Return tshark's stdout stream or fail when capture output was not piped."""
...
def parse_capture_line(line: str) -> CapturedUdpPayload | None:
    """Decode one tshark port-and-hex-payload record, ignoring malformed lines."""
    stripped = line.strip()
    if not stripped:
        return None
    try:
...
def relay_payload(
    ...
    """Forward decoded capture records until the producer reaches EOF."""
...
def log_relay_stats_if_due(args: argparse.Namespace, state: RelayState) -> None:
    """Emit relay counters only when the reporting interval has elapsed."""
    now = time.monotonic()
...
def close_relay_sockets(sockets: RelaySockets) -> None:
    """Close both nonblocking UDP sockets owned by the relay."""
    sockets.video.close()
    sockets.audio.close()
...
async def run_relay(args: argparse.Namespace) -> int:
    ...
    """Run tshark capture and UDP forwarding until shutdown or a relay failure."""
...
def main() -> int:
    """Run the relay until completion, treating an operator interrupt as clean shutdown."""
    try:
